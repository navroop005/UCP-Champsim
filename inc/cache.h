...
class CACHE : public MEMORY {
  public:
    ...
    CACHE(string v1, uint32_t v2, int v3, uint32_t v4, uint32_t v5, uint32_t v6, uint32_t v7, uint32_t v8) 
        ...
        if (NAME == "LLC")
        {
            llc_ucp = new UCP((uint32_t)NUM_CPUS, (uint32_t)LLC_WAY, (uint32_t)LLC_SET);
        }
    ...
    void add_mshr(PACKET *packet),
         ...
         llc_update_replacement_state(uint32_t cpu, uint32_t set, uint32_t way, uint64_t full_addr, uint64_t ip, uint64_t victim_addr, uint32_t type, uint8_t hit, uint64_t packet_addr),
    ...
    uint32_t get_set(uint64_t address),
             ...
             lcc_lru_victim(uint32_t cpu, uint64_t instr_id, uint32_t set, const BLOCK *current_set, uint64_t ip, uint64_t full_addr, uint32_t type);
